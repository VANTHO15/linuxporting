...
struct kobj_attribute m_attr = __ATTR(foo_value, 0660, sysfs_show, sysfs_store);
...
{
        pr_info("Sysfs read.....\n");
        return sprintf(buf, "%d", foo_value);
}
...
{   
    ...
    /* 06.1 Create proc directory. It will create a directory under "/sys" */
    mdev.kobj_ref = kobject_create_and_add("m_sysfs", NULL);

    /* 06.2 Creating Proc entry under "/sys/m_attr/" */
...
{
    kobject_put(mdev.kobj_ref);                     /* 06.2 */
    sysfs_remove_file(mdev.kobj_ref , &m_attr.attr);         /* 06.1 */
